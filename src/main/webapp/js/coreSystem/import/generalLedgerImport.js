$(document).ready(function () {
    ...
    var generalLedger;
    var itemSelected = {};
    var url = "json/generalLedger.json";
    ...
    generalLedger = initGrid(url,"generalLedger",colModel, colNames,
        function (id) {
    ...
    //重新加载
    function reloadgeneralLedger() {
        var subjectNo = $("#nameQuery").val() == undefined ? '' :$("#nameQuery").val()
        generalLedger.clearGridData().prev().remove();  // 清空数据
        generalLedger.setGridParam({  // 重新加载数据
            mtype: "GET",
            datatype: "json",
            url: url,
            page: 1
        ...
        add: function (e, data) {
          ...
          if (data.originalFiles[0]['type'].length && !acceptFileTypes.test(data.originalFiles[0]['type'])) {
            ...
            if(fileName.indexOf("txt") > 0|| fileName.indexOf("xlsx") > 0|| fileName.indexOf("xls") > 0){
                ...
                generalLedger.addRowData(data.id , data);
            }else{
