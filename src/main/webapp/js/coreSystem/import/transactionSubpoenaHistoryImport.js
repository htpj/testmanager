$(document).ready(function () {
    ...
    var subpoenaHistoryParam;
    var itemSelected = {};
    var url = "json/transactionSubpoenaHistoryImport.json";
    ...
    subpoenaHistoryParam = initGrid(url,"subpoenaHistoryParam",colModel, colNames,
        function (id) {
    ...
    $("#queryBtn").on("click", function () {
        reloadsubpoenaHistoryParam();
    });

    //重新加载
    function reloadsubpoenaHistoryParam() {
        var subjectNo = $("#nameQuery").val() == undefined ? '' :$("#nameQuery").val()
        subpoenaHistoryParam.clearGridData().prev().remove();  // 清空数据
        subpoenaHistoryParam.setGridParam({  // 重新加载数据
            mtype: "GET",
            datatype: "json",
            url: url,
            page: 1
        ...
        add: function (e, data) {
          ...
          if (data.originalFiles[0]['type'].length && !acceptFileTypes.test(data.originalFiles[0]['type'])) {
            ...
            if(fileName.indexOf("txt") > 0|| fileName.indexOf("xlsx") > 0|| fileName.indexOf("xls") > 0){
                ...
                subpoenaHistoryParam.addRowData(data.id , data);
            }else{
