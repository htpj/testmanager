$(document).ready(function () {
    ...
    var externalAccountBalance;
    var itemSelected = {};
    var url = "json/externalAccountBalanceImport1.json";
    ...
    externalAccountBalance = initGrid(url,"externalAccountBalance",colModel, colNames,
        function (id) {
    ...
    $("#queryBtn").on("click", function () {
        reloadexternalAccountBalance();
    });

    //重新加载
    function reloadexternalAccountBalance() {
        var subjectNo = $("#nameQuery").val() == undefined ? '' :$("#nameQuery").val()
        externalAccountBalance.clearGridData().prev().remove();  // 清空数据
        externalAccountBalance.setGridParam({  // 重新加载数据
            mtype: "GET",
            datatype: "json",
            url: url,
            page: 1
        ...
        add: function (e, data) {
          ...
          if (data.originalFiles[0]['type'].length && !acceptFileTypes.test(data.originalFiles[0]['type'])) {
            ...
            if(fileName.indexOf("txt") > 0|| fileName.indexOf("xlsx") > 0|| fileName.indexOf("xls") > 0){
                ...
                externalAccountBalance.addRowData(data.id , data);
            }else{
