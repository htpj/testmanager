$(document).ready(function () {
    ...
    var internalAccountBalance;
    var itemSelected = {};
    var url = "json/internalAccountBalanceImport.json";
    ...
    internalAccountBalance = initGrid(url,"internalAccountBalance",colModel, colNames,
        function (id) {
    ...
    $("#queryBtn").on("click", function () {
        reloadinternalAccountBalance();
    });

    //重新加载
    function reloadinternalAccountBalance() {
        var subjectNo = $("#nameQuery").val() == undefined ? '' :$("#nameQuery").val()
        internalAccountBalance.clearGridData().prev().remove();  // 清空数据
        internalAccountBalance.setGridParam({  // 重新加载数据
            mtype: "GET",
            datatype: "json",
            url: url,
            page: 1
        ...
        add: function (e, data) {
          ...
          if (data.originalFiles[0]['type'].length && !acceptFileTypes.test(data.originalFiles[0]['type'])) {
            ...
            if(fileName.indexOf("txt") > 0|| fileName.indexOf("xlsx") > 0|| fileName.indexOf("xls") > 0){
                ...
                internalAccountBalance.addRowData(data.id , data);
            }else{
