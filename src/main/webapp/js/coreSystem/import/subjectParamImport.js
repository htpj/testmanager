$(document).ready(function () {
    ...
    var jqGridSubjectParam;
    var itemSelected = {};
    var url = "json/subjectParamImport.json";
    ...
    jqGridSubjectParam = initGrid(url,"jqGridSubjectParam",colModel, colNames,
        function (id) {
    ...
    $("#queryBtn").on("click", function () {
        reloadjqGridSubjectParam();
    });

    //重新加载
    function reloadjqGridSubjectParam() {
        var subjectNo = $("#nameQuery").val() == undefined ? '' :$("#nameQuery").val()
        jqGridSubjectParam.clearGridData().prev().remove();  // 清空数据
        jqGridSubjectParam.setGridParam({  // 重新加载数据
            mtype: "GET",
            datatype: "json",
            url: url,
            page: 1
        ...
        add: function (e, data) {
          ...
          if (data.originalFiles[0]['type'].length && !acceptFileTypes.test(data.originalFiles[0]['type'])) {
            ...
            if(fileName.indexOf("txt") > 0|| fileName.indexOf("xlsx") > 0|| fileName.indexOf("xls") > 0){
                ...
                jqGridSubjectParam.addRowData(data.id , data);
            }else{
